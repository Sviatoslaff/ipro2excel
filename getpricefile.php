...
/*
	for ($row = 2; $row <= $highestRow; ++$row) {
        $article = $sheet->getCellByColumnAndRow($articleCol, $row)->getValue();
		setGoodsProperties($session, $sheet, $article, $row);
		setPriceProperties($session, $sheet, $article, $row);
		setQuantityProperties($session, $sheet, $article, $row);
        echo 'Обработан ' . $article . '<br>' . PHP_EOL;
		if ($row % 1000 == 0 ) 
...
function setQuantityProperties($session, $article, $row) {
	...
	if ($res_array["status"]["code"] == "200") {
		...
		foreach ($fields as $fieldnum => $field) {
			...
			if (count($arrKeys) == 1) {	
				if ($arrKeys[0] == "StoreQuantRem") {		
					$arrMeas = $arrData;
					foreach ($arrMeas as $store) {
						switch ($store["StoreType"]) {
							case "rc" :
								$row->setCellAtIndex(WriterEntityFactory::createCell($store[$field]), $colindex);	
								break;
							}
					}
				}
...
function makeRequest($article, $session, $querytype) {
	...
	$res = file_get_contents($source .'goods/'.$article. $querytype . '?type=mnf&session-id='.$session);
