...
ETM export to Excel files. Описание ETM API - в каталоге docs.
...
4. Нужно дать права www-пользователя и www-группы на папку tmp в каталоге etm2excel. Альтернатива: в config.ini указать уже имеющуюся доступную папку для временных файлов. Компонент box/scout использует temp-папку из настроек сервера.
...
- Результат сохранится в виде файла exportgoods.xlsx (название меняется в config.ini, ключ xlsxgoodsfilename). 1.4. Скачать файл можно по url http://вашсайт.ру/ipro2excel/tmp/exportgoods.xlsx
